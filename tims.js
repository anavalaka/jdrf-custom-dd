...
const company_dd_id = '#attribute1';
const division_dd_id = '#attribute4';

const attrWaiter = setInterval(() => {
  if (document.querySelector(company_dd_id) && document.querySelector(division_dd_id) && document.querySelector(division_dd_id).closest('rs-field-input')) {
    clearInterval(attrWaiter);
    ...
    const attr4 = document.querySelector(division_dd_id).closest('rs-field-input');
    const attr4shownDisplay = attr4.style.display;
    attr4.style.display = "none";

    const company_dd = document.querySelector(company_dd_id);
    if (company_dd.matches('select')) {
      const onSelectChange = () => {
        setTimeout(() => {
          const attr1Selected = company_dd.value ?? '';
      ...
      company_dd.addEventListener('change', onSelectChange);
      return;
    }

    company_dd.closest('.mat-mdc-form-field-flex').addEventListener('click', () => {
      setTimeout(() => {
        const anyClicker = () => {
          console.log('any clicked');
          setTimeout(() => {
            const attr1Selected = company_dd.value ?? company_dd.querySelector('.mat-mdc-select-value')?.innerText ?? '';
            console.log('attr1Selected', attr1Selected);
            if (attr1Selected) {
              const hasMatch = __custom_dd_groups_options.some(x => x[0] == attr1Selected);
              attr4.style.display = hasMatch ? attr4shownDisplay : 'none';
            ...
            document.removeEventListener('click', anyClicker);
          }, 100);
        };
        console.log('attr1 clicked');
        document.addEventListener('click', anyClicker, {
          once: true
        });
      }, 1);
    });
...
var $$find = function (selector, fn, n) {
  var el = document.querySelector(selector);
  ((+n || 0) < 100)
    ? (!el ? setTimeout(function () { $$find(selector, fn, (n || 0) + 1) }, 100) : fn(el))
    : console.log("Not found: ", selector);
};


window.setTimeout(function () {
  $$find(division_dd_id, function (div) {
    const wrapper = div.closest('.mat-mdc-text-field-wrapper');
    wrapper.addEventListener("click", function () {
      var selectedSpan = document.querySelector(`${company_dd_id} > div > div > span > span:first-child`) || { innerText: "###" };
      if (selectedSpan) {
        var selectedText = document.querySelector(`select${company_dd_id}`)?.value ?? selectedSpan.innerText;
        console.log('selectedText', selectedText)
        $$find(`${division_dd_id}-panel, select${division_dd_id}`, function (pnl) {
          for (var i = 0; i < pnl.children.length; ++i) {
            var child = pnl.children[i];
            var span =  child.matches('option')? child : child.querySelector("span");
            if (span) {
