...
wany.__initChildDropdown = (parent: HTMLElement, removed$: Observable<any>) => {
  const rsField = parent.closest('rs-field')!;
  const divisionDD = rsField.querySelector(division_dd_id)!;
  const wrapper = divisionDD.closest('.mat-mdc-text-field-wrapper')!;
